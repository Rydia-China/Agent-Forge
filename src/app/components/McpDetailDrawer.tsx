"use client";

import { Drawer, Button, Alert, Spin, Typography, Space } from "antd";
import { DeleteOutlined } from "@ant-design/icons";
import type { UseMcpDetailReturn } from "./hooks/useMcpDetail";
import { SkillEditor } from "./SkillEditor";
import { McpEditor } from "./McpEditor";

export interface McpDetailDrawerProps {
  detail: UseMcpDetailReturn;
}

export function McpDetailDrawer({ detail }: McpDetailDrawerProps) {
  const showDelete =
    (detail.selectedMcp?.type === "skill" &&
      detail.skillDetail &&
      detail.skillDetail.productionVersion > 0) ||
    (detail.selectedMcp?.type === "mcp" &&
      detail.mcpDetail &&
      detail.mcpDetail.productionVersion > 0);

  return (
    <Drawer
      title={
        detail.selectedMcp ? (
          <Space>
            <Typography.Text strong>
              {detail.selectedMcp.type === "skill" ? "Skill" : "MCP"}
            </Typography.Text>
            <Typography.Text type="secondary">·</Typography.Text>
            <Typography.Text>{detail.selectedMcp.name}</Typography.Text>
          </Space>
        ) : "Detail"
      }
      placement="right"
      styles={{ wrapper: { width: '90vw' }, body: { maxWidth: 1400 } }}
      open={!!detail.selectedMcp}
      onClose={() => detail.setSelectedMcp(null)}
      extra={
        showDelete ? (
          <Button
            danger
            icon={<DeleteOutlined />}
            loading={detail.isDeletingMcp}
            onClick={() => void detail.deleteSelectedMcp()}
          >
            Delete
          </Button>
        ) : null
      }
    >
      {detail.error && <Alert type="error" title={detail.error} showIcon closable style={{ marginBottom: 12 }} />}
      {detail.notice && <Alert type="success" title={detail.notice} showIcon closable style={{ marginBottom: 12 }} />}

      {/* Prefer showing cached content over spinner to avoid flash on reopen.
          Only fall back to spinner when no cached data exists. */}
      {detail.selectedMcp?.type === "skill" && detail.skillDetail ? (
        <SkillEditor
          detail={detail.skillDetail}
          versions={detail.skillVersions}
          edit={detail.skillEdit}
          setEdit={detail.setSkillEdit}
          isSaving={detail.isSavingMcp}
          isPublishing={detail.isPublishingVersion}
          onSave={() => void detail.saveSkillVersion()}
          onPublish={(v) => void detail.publishSkillVersion(v)}
        />
      ) : detail.selectedMcp?.type === "mcp" && detail.mcpDetail ? (
        <McpEditor
          detail={detail.mcpDetail}
          versions={detail.mcpVersions}
          edit={detail.mcpEdit}
          setEdit={detail.setMcpEdit}
          isSaving={detail.isSavingMcp}
          isPublishing={detail.isPublishingVersion}
          onSave={() => void detail.saveMcpVersion()}
          onPublish={(v) => void detail.publishMcpVersion(v)}
        />
      ) : detail.isLoadingMcpDetail ? (
        <div className="flex items-center justify-center py-12">
          <Spin description="Loading…" />
        </div>
      ) : (
        <Typography.Text type="secondary">No detail loaded.</Typography.Text>
      )}
    </Drawer>
  );
}

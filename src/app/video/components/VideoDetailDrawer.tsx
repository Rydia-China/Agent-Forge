...
export function VideoDetailDrawer({ keyResourceId, onClose, onRefresh, sessionId }: VideoDetailDrawerProps) {
  ...
  const fetchDetail = useCallback(async (id: string, silent = false) => {
    ...
    try {
      ...
      void message.error("Failed to load video detail");
    } finally {
      setIsLoading(false);
    }
  }, [message]);
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      ...
      void message.success("Video regenerated");
      void fetchDetail(detail.id, true);
      onRefresh?.();
    } catch {
  ...
  }, [detail, editPrompt, promptDirty, fetchDetail, message, onRefresh, sessionId]);
    ...
    >
      {isLoading || !detail ? (
        ...
        <div className="flex gap-5" style={{ height: "calc(100vh - 110px)" }}>
          {/* ── Left Column: Video + Version History ── */}
          <div className="flex w-1/2 flex-col gap-3">
            ...
            <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-lg border border-slate-700 bg-slate-900/30">
              {viewedVerRow?.url ? (
                ...
                <span className="text-sm text-slate-500">No video yet</span>
              )}
            </div>
          ...
          <div className="flex w-1/2 flex-col gap-3">
            ...
            <div className="flex shrink-0 flex-wrap gap-2">
              ...
              <Tooltip title="Video generation takes 2–4 minutes">
                <Button
                  type="primary"
                  size="small"
                  icon={<ReloadOutlined />}

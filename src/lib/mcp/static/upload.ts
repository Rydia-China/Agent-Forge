...
export const RequestUploadParams = z.object({
...
export interface UploadRequest {
  ...
  purpose?: string;
  maxSizeMB?: number;
  /** JSON body template. Values may contain placeholders: {{fileContent}}, {{fileName}}, {{timestamp}} */
...
/**
 * Return a CallToolResult that carries an `_uploadRequest` side-channel.
 * The agent layer inspects this field and emits an SSE event to the frontend.
 * LLM only sees `{ uploadId, status: "pending" }`.
 */
function uploadResult(req: UploadRequest): CallToolResult {
  const result = text(
    JSON.stringify({ uploadId: req.uploadId, status: "pending" }),
  );
  // Side-channel: agent layer reads this to emit SSE
...
export const uploadMcp: McpProvider = {
  name: "upload",
    ...
    return [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            timeout: {
              type: "number",
              description: "Request timeout in seconds. Default: 60. Set higher for large file uploads.",
            },
          },
          required: ["endpoint"],
        },

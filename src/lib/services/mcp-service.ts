...
const CORE_MCP_NAMES = new Set(["mcp_manager", "ui", "memory", "sync"]);
...
const CORE_MCPS: readonly string[] = ["mcp_manager", "ui", "memory", "sync"];
...
export function listStaticMcpProviders(): StaticMcpSummary[] {
  const isActive = (name: string) => !!registry.getProvider(name);

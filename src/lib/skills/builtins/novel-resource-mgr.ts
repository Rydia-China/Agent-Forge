...
description: Initialize and manage novel-level shared resources (character portraits, scene images). Use when working on novel-wide resource setup, character initialization, or scene location management.
tags:
  - video
  - resource
  - novel
...
这些资源的 \`scopeType\` 必须为 \`"novel"\`，将被所有 EP 共享复用。

## 与 EP 级操作的区分

- **小说级（本 skill）** — 初始化全局资源，scopeType = "novel"
- **EP 级（video-workflow）** — 分镜制作、视频生成，scopeType = "script"

## 工作流程

### 1. 初始化小说资源

当用户上传小说或初次设置时，应从小说内容中提取：
...
\\\`\\\`\\\`json
{
  "items": [{
    "key": "scene_school_bg",
    "prompt": "现代高中校园，操场上有樱花树，蓝天白云，动漫背景风格",
    "category": "场景",
    "scopeType": "novel",
    "scopeId": "novel-uuid-from-context",
    "title": "学校操场"
...
- **修改** — 重新生成（相同 key 会创建新版本）
- **删除** — 目前需通过 UI 操作

### 3. 与 EP 级的协作

- 小说级资源初始化完成后，EP 级工作流（分镜制作）可以直接引用这些资源
...
- **检查重复** — 生成前查看 Image Registry，避免重复生成已有资源
...
- \`generate_image\` — 生成图片（自动持久化到 domain_resources）

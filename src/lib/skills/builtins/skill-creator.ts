...
Agent 看到 description 判断相关后，才通过 \`get_skill\` 读取全文。
...
4. **选择 tags** — 用于分类，保持简洁。
5. **创建** — 调用 \`skill_admin__create\` 写入数据库。
6. **验证** — 调用 \`get_skill\` 确认内容正确持久化。

## 可用工具

| 操作 | 工具 |
|------|------|
|| 列出所有 skill | \`list_skills\` |
| 读取全文 | \`get_skill\` |
| 创建 | \`skill_admin__create\` |
| 更新 | \`skill_admin__update\` |
| 删除 | \`skill_admin__delete\` |
| 导入 SKILL.md | \`skill_admin__import\` |
| 导出 SKILL.md | \`skill_admin__export\` |

## SKILL.md 互操作

本系统兼容 Agent Skills 开放标准的 SKILL.md 格式（YAML frontmatter + Markdown body）。

导入：将 SKILL.md 原文传给 \`skill_admin__import\`，系统自动解析 frontmatter 中的 name/description 和 body 中的 content。
导出：\`skill_admin__export\` 返回标准 SKILL.md 格式，可直接用于 Claude Code / Codex 等工具。

## 业务表结构声明（requiredSchemas）

如果 skill 依赖业务数据库表，**必须**在 metadata 中通过 \`requiredSchemas\` 声明完整的表结构定义。
这是表结构的权威源。当 agent 发现表结构与定义不一致时，可以对照定义自行修复。
    ...
    "requiredSchemas": [
      {
        ...
        "constraints": [{ "type": "pk", "columns": ["id"] }]
      }
    ]
  }
}
...
**用途**：
- Agent 读取 skill 后知道“这张表应该长什么样”
- 表不存在时，agent 可用 \`biz_db__create_table\` 按定义创建
- 表结构漂移时，agent 可用 \`biz_db__diff_schema\` 检测，再用 \`biz_db__alter_table\` 修复
- 迁移新环境时，agent 可用 \`biz_db__ensure_schema\` 重建

**规则**：
- 格式与 \`biz_db__create_table\` 的输入完全一致（columns + constraints）
- 当 skill 的表结构变化时，同时更新 metadata 中的 requiredSchemas
- 每张业务表都应只在一个 skill 中声明（单一权威源）

...
4. service 按 JSON 内容初始化 versioned `KeyResource` 占位；有真实子地点的父地点本身不是交付图，不创建同名父地点单图 `scene_<父地点名>`；只有真实子地点 >= 2 时才额外创建 `scene_<父地点名>_grid`
...
2. 普通地点输出 `scene_<地点名>`
3. 有真实子地点的父地点本身不是实际场景单图，必须忽略同名 `scene_<父地点名>`；父地点只有在没有真实子地点时才可作为普通地点输出 `scene_<地点名>`
